...
class LedgerDBRouter(object):
    ...
    def db_for_read(self, model, **hints):
        if (
            model._meta.db_table == "accounts_emailuser"
            or model._meta.db_table == "address_country"
            or model._meta.db_table == "payments_invoice"
            or model._meta.db_table == "accounts_emailuser_documents"
            or model._meta.db_table == "accounts_document"
            or model._meta.db_table == "accounts_emailidentity"
            or model._meta.db_table == "basket_basket"
            or model._meta.db_table == "accounts_emailuser_user_permissions"
        ):
            return "ledger_db"
        if (
            model._meta.db_table == "auth_group"
        ):  # or model._meta.db_table == 'auth_permission':
            return "ledger_db"
    ...
    def db_for_write(self, model, **hints):
        ...
        if (
            model._meta.db_table == "accounts_emailuser"
        ):  # or model._meta.db_table == 'auth_group' or model._meta.db_table == 'auth_permission':
            return "ledger_db"
        return None
    ...
    def allow_relation(self, obj1, obj2, **hints):
        ...
        """
        if (
            "accounts_emailuser" == obj1._meta.db_table
            and "parkstay_campgroundgroup_members" == obj2._meta.db_table
        ):
            return True
...
class Address(BaseAddress):
    user = models.ForeignKey(
        "EmailUserRO", related_name="profile_addresses", on_delete=models.DO_NOTHING
    )
    oscar_address = models.ForeignKey(
        UserAddress, related_name="profile_addresses", on_delete=models.DO_NOTHING
    )
...
class PermissionsMixinRO(models.Model):
    ...
    def get_system_group_permission(self, user_id):
        ...
        cache_name_sgp = (
            "managed_models.SystemGroupPermission.objects.filter(emailuser_id="
            + str(user_id)
            + ")"
        )
        sgp_groups = []
        ...
        if sgp_cache is None:
            sgp = managed_models.SystemGroupPermission.objects.filter(
                emailuser_id=self.id
            )
            for s in sgp:
    ...
    def system_group_permision_list(self, system_group_id):
        ...
        permission_list = []
        cache_name_pl = (
            "managed_models.SystemGroup.objects.filter(id=" + str(system_group_id) + ")"
        )
        pl_cache = cache.get(cache_name_pl)
        if pl_cache is None:
            system_groups = managed_models.SystemGroup.objects.filter(
                id=system_group_id
            )
        ...
        def all(self, **kwargs):
            try:
                url = (
                    ...
                    + "/ledgergw/remote/user-groups/"
                    + str(self.emailuser_id)
                    + "/"
                ...
                raise ValidationError(
                    "Error: Unable to create user - Issue connecting to ledger gateway"
                )
            return self
...
class EmailUserRO(AbstractBaseUser, PermissionsMixinRO):
    ...
    residential_address = models.ForeignKey(
        Address, null=True, blank=False, related_name="+", on_delete=models.DO_NOTHING
    )
    postal_address = models.ForeignKey(
        Address, null=True, blank=True, related_name="+", on_delete=models.DO_NOTHING
    )
    ...
    identification2 = models.ForeignKey(
        PrivateDocument,
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
        related_name="identification_document_2",
    )

    senior_card = models.ForeignKey(
        Document,
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
    ...
    senior_card2 = models.ForeignKey(
        PrivateDocument,
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
    ...
    def __str__(self):
        if self.is_dummy_user:
            if self.organisation:
                return "{} {} ({})".format(
                    self.first_name, self.last_name, self.organisation
                )
    ...
    def get_full_name_dob(self):
        full_name_dob = "{} {} ({})".format(
            self.first_name, self.last_name, self.dob.strftime("%d/%m/%Y")
        )
    ...
    def is_dummy_user(self):
        return (
            not self.email
            or self.email[-1 * self.dummy_email_suffix_len :] == self.dummy_email_suffix
        )
    ...
    def get_dummy_email(self):
        ...
        return "{}.{}.{}{}".format(
            self.first_name, self.last_name, uid, self.dummy_email_suffix
        )
    ...
    def is_senior(self):
        ...
        """
        return (
            self.dob < date(1955, 7, 1)
            or (
                (date(1955, 7, 1) <= self.dob <= date(1956, 6, 30)) and self.age() >= 61
            )
            or (
                (date(1956, 7, 1) <= self.dob <= date(1957, 6, 30)) and self.age() >= 62
            )
            or (
                (date(1957, 7, 1) <= self.dob <= date(1958, 6, 30)) and self.age() >= 63
            )
            or (
                (date(1958, 7, 1) <= self.dob <= date(1959, 6, 30)) and self.age() >= 64
            )
            or (self.dob > date(1959, 6, 1) and self.age() >= 65)
        )
...
class Basket(models.Model):
    ...
    owner = models.ForeignKey(
        "EmailUserRO",
        related_name="owner_basket_name",
        null=True,
